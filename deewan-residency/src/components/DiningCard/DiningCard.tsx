  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-300 border border-gray-100 flex flex-col h-full group/card">
      ...
      <div className="p-6 flex-1 flex flex-col">
        <div className="mb-6">
             <p className={`text-gray-600 leading-relaxed font-grotesk ${!isExpanded ? 'line-clamp-3' : ''}`}>
        ...
        <div className="space-y-4 mb-6">
          <div className="flex items-center text-sm text-gray-600 font-grotesk">
            ...
            <div>
                <span className="block text-xs font-bold text-gray-400 uppercase tracking-wider">Opening Hours</span>
                <span className="font-medium text-gray-900">{diningOption.operatingHours}</span>
            </div>
          </div>
          ...
          <div className="flex items-center text-sm text-gray-600 font-grotesk">
            ...
            <div>
                <span className="block text-xs font-bold text-gray-400 uppercase tracking-wider">Contact</span>
                <a href={`tel:${diningOption.contactInfo}`} className="font-medium text-gray-900 hover:text-amber-600 transition-colors">
                ...
                <div>
                  ...
                  <ul className="grid grid-cols-1 gap-3">
                    {diningOption.features.map((feature, index) => (
                      <li key={index} className="text-sm text-gray-600 flex items-start font-grotesk bg-gray-50 p-2 rounded-lg">
                    ...
                    {diningOption.menuHighlights.slice(0, 4).map((item) => (
                      <div key={item.id} className="group/item">
                          <div className="flex items-center justify-between mb-1">
                            <span className="text-base font-sinoreta text-gray-900 tracking-wide group-hover/item:text-amber-600 transition-colors">{item.name}</span>
                             <div className="flex gap-1">
                                {item.isVegetarian && (
                                  <span className="w-2 h-2 rounded-full bg-green-500 ring-2 ring-green-100" title="Vegetarian"></span>
                                )}
                                {item.isSpicy && (
                                  <span className="w-2 h-2 rounded-full bg-red-500 ring-2 ring-red-100" title="Spicy"></span>
                                )}
                             </div>
                          </div>
                          <p className="text-xs text-gray-500 font-grotesk leading-relaxed">{item.description}</p>
                      </div>
                    ))}
                  </div>
                </div>
